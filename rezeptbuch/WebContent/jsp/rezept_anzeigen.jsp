		...
		<c:when test="${not empty rezept}">
			...
			<div id="Einstellungen">
				...
				<c:if test="${not empty user and user.id eq rezept.creator.id}">
					<button id="editRecipeButton" name="editRecipeButton"
						class="button" type="button">Rezept bearbeiten</button>
				</c:if>
				...
				<form id="editForm" action="/rezeptbuch/EditRecipeServlet"
						...
						<c:forEach var="ingredient" items="${rezept.ingredients}">
							<div class="newLine clearfix">
								<input type="number" name="zutatenMenge" class="zutatenMenge"
									placeholder="Menge" required value="${ingredient.quantity}">
								<select name="zutatenEinheit" class="zutatenEinheit"></select> <input
									type="text" name="zutatenZutat" class="zutatenZutat"
									placeholder="Zutat" required value="${ingredient.ingredient}">
					...
					<div id="editRecipe">
						...
						<label for="durationPreparation">Vorbereitungszeit</label> <input
							...
							value="${rezept.durationPreparation}"> <label
							for="durationCooking">Kochzeit</label> <input disabled
							type="number" name="durationCooking" id="durationCooking" min="0"
							max="500" value="${rezept.durationCooking}"> <label
							for="difficulty">Schwierigkeitsgrad</label> <input disabled
							type="number" name="difficulty" id="difficulty" max="5" min="0"
							value="${rezept.difficulty}"> <label for="servings">Portionen</label>
						<input disabled type="number" name="servings" id="servings"
							max="99" min="0" value="${rezept.servings}">
				...
				<c:choose>
					<c:when test="${not empty user }">
						<form id="rating-form" name="rating-form"
							action="/rezeptbuch/ratingservlet" method="post"
							autocomplete="off">
							...
							<fieldset class="rating" form="rating-form">
								<input class="star-radio" type="radio" id="star5" name="rating"
									...
									name="rating" value="3" /> <label class="star-label"
									for="star3" title="meh">3 stars</label> <input
									class="star-radio" type="radio" id="star2" name="rating"
									value="2" /> <label class="star-label" for="star2"
									title="kinda bad">2 stars</label> <input class="star-radio"
									type="radio" id="star1" name="rating" value="1" /> <label
									class="star-label" for="star1" title="sucks big time">1
									star</label>
							</fieldset>
			...
			<div id="Kommentare">
				...
				<c:if test="${not empty user}">
					<form id="commentForm">
						<p>
							<label class="labelfortextarea" for="newComment">Verfassen
								sie einen neuen Kommentar </label>
							<textarea name="newComment" id="newComment"
								placeholder="Kommentar verfassen..." maxlength="2500" required></textarea>
						</p>
